...
MathLinkCompilerAdditionsPath::usage = ""
(*** Messages ***)
...
GetOption[driver_, errHd_Symbol, opts_List, "IncludePath", targetSystemID_] :=
	With[{sysincpath = GetTypeCheckedOption[driver, opts, 
		...
		ResolveIncludeDirs[sysincpath, incpath, targetSystemID]
	]
...
GetOption[driver_, errHd_Symbol, opts_List, n:"SystemLibraries", 
	...
	ResolveLibraries[driver, 
		GetTypeCheckedOption[driver, opts, n, 
			...
			Message[errHd::sysfilelist, n, #]&], targetSystemID, method]

GetOption[driver_, errHd_Symbol, opts_List, n:"SystemLibraryDirectories", 
	targetSystemID_] :=
	ResolveLibraryDirectories[GetTypeCheckedOption[driver, opts, n, 
...
(******************************************************************************)
InvokeCompiler[driver_, compileFn_, ext_, code:(_String|{_String ..}), 
...
InvokeCompilerImpl[driver_, compileFn_, errMsgHd_, ext_, 
	code:(_String|{_String ..}), funName_String, opts_List] := 
	Module[{outputBaseName, outputFileName, compilerInstallation, compilerName, 
		...
		mprepOptions, createBinary, language, buildScript, linkWithMathLinkQ,
		libType},
		...
		*)
		...
		compileCommands = createCompileCommands[driver, compileFn, libType,
			...
			targetSystemID, cleanIntermediate, mprepOptions, language, opts];
...
(******************************************************************************)
MprepCalls[tmSrcFiles_List, workDir_String, mprepOptions_] := 
	Map[
		Function[tmSrc,
			MprepCall[MathLinkTemplateOutputFile[workDir, tmSrc], tmSrc, 
				mprepOptions]],
		tmSrcFiles
	]

(******************************************************************************)
MprepCall[outFile_, tmFile_, mprepOptions_] := 
(
	MarkFileForCleanup[outFile];
	StringJoin[QuoteFile[$MprepExePath], 
		" -o ", QuoteFile[outFile], 
...
(******************************************************************************)
$MprepExePath = 
	FileNameJoin[{MathLinkCompilerAdditionsPath[$SystemID]} ~Join~ 
		Switch[$SystemID,
			"Windows", {"mldev32", "bin", "mprep.exe"},
			"Windows-x86-64", {"mldev64", "bin", "mprep.exe"},
			_, {"mprep"}
		]
	]
...
BaseDriver["ResolveCodeFile"][errHd_, srcFiles:{_String ..}, workDir_String, 
...
(******************************************************************************)
ResolveIncludeDirs[Automatic, extDirs_, targetSystemID_, opts___?OptionQ] := 
	ResolveIncludeDirs[
		{
			FileNameJoin[{$InstallationDirectory, 
				"SystemFiles", "IncludeFiles", "C"}],
			FileNameJoin[{$InstallationDirectory, 
				"SystemFiles", "Links", "MathLink", "DeveloperKit"} ~Join~ 
					MathLinkIncDir[targetSystemID]]
		},
		extDirs]
...
MathLinkIncDir::system = "TargetSystemID `1` is not a valid $SystemID value";
MathLinkIncDir[sysid_] := 
	Switch[sysid,
		"Windows", {sysid, "CompilerAdditions", "mldev32", "include"},
		"Windows-x86-64", 
			{sysid, "CompilerAdditions", "mldev64", "include"},
		_String , {sysid, "CompilerAdditions"},
		$Failed, {},
		_, Message[MathLinkIncDir::system, sysid]; {}]
...
ResolveLibraries[driver_, Automatic, 
	target:("MacOSX-x86" | "MacOSX-x86-64"), False] :=(
	ResolveLibraries[driver, Automatic, target, True]
);

ResolveLibraries[driver_, Automatic, targetSystemID_, False] :=	{}

ResolveLibraries[driver_, Automatic, targetSystemID_, True] := 
	driver["ResolveSystemLibraries"][targetSystemID] /. {
		lib_String :> {lib},
		libs_List :> libs
	}

ResolveLibraries[driver_, All | True, targetSystemID_, _] := 
	driver["ResolveSystemLibraries"][targetSystemID] /. {
		lib_String :> {lib},
		libs_List :> libs
	}

BaseDriver["ResolveSystemLibraries"][targetSystemID_] :=
	MathLinkLib[targetSystemID]

$MathLinkDeveloperKit = {$InstallationDirectory, "SystemFiles", "Links", 
	"MathLink", "DeveloperKit"}

ResolveLibraries[driver_, libDir_String, targetSystemID_, exeQ_] := 
	ResolveLibraries[driver, {libDir}, targetSystemID, exeQ]

ResolveLibraries[driver_, libDirs:{_String ...}, _, _] := libDirs
...
(******************************************************************************)
ResolveLibraryDirectories0[Automatic, sysid_, opts:OptionsPattern[]]:=
	{
		FileNameJoin[{MathLinkCompilerAdditionsPath[sysid]} ~Join~ 
			Switch[sysid,
				"Windows", {"mldev32", "lib"},
				"Windows-x86-64", {"mldev64", "lib"},
				"Linux" | "Linux-x86-64" | "Linux-ARM" | "MacOSX-x86-64", {},
				$Failed, {},
				_, Message[MathLinkLib::system, sysid]; {}
			]
		],
		FileNameJoin[{$InstallationDirectory, "SystemFiles","Libraries", sysid}]
	}

ResolveLibraryDirectories[Automatic, sysid_, opts:OptionsPattern[]] :=
	(* 
		Determine whether we should use the library paths 
		or just use the framework path, defined in GCCCompiler SystemCompileOptions.
	*)
	If[MatchQ[sysid,"MacOSX-x86-64"],
		If[!MatchQ["SystemLibraries" /.opts /. "SystemLibraries"->False, False],
			ResolveLibraryDirectories0[Automatic, sysid, opts],
			{"-F"<>MathLinkCompilerAdditionsPath[sysid]}],
		ResolveLibraryDirectories0[Automatic, sysid, opts]
	]

ResolveLibraryDirectories[dir_String, t_, _] := ResolveLibraryDirectories[{dir}, t]

ResolveLibraryDirectories[dirs_List, ___] := dirs
