...
$AlphaQueryBaseURL[url_] :=
	Block[{baseurl},
		...
		baseurl = Switch[ ToLowerCase[baseurl],
				...
				"test2", "Test2" /. Map[Reverse, $AlphaQueryBaseURLs],
				...
				"maps", "Maps" /. Map[Reverse, $AlphaQueryBaseURLs],
...
CreateDialog[{
	openerPanel[{
		"Server",
		Row[{
			SetterBar[dynamicPreference["BaseURL", "Automatic"], DeleteCases[Last /@ $AlphaQueryBaseURLs, "Maps"]],
...
Block[{assumptions, nonformula, formula},
	...
	nonformula = normalizeAssumption /@ Cases[assumptions,
		XMLElement["assumption", {___, "type" -> (Alternatives @@ $NoneFormulaAssumptionTypes), ___}, _] ];
...
WolframAlpha[str_String, "URL", opts:OptionsPattern[]] := 
	Block[{baseurl, baseparams, async, format, assumption, includepodid, excludepodid,
		...
		If[StringQ["SubstituteURL" /. methodOptions], Return["SubstituteURL" /. methodOptions]];
...
WolframAlpha[str_String, "ToQuantity", opts:OptionsPattern[]] :=
	Block[{$AlphaQueryMMode = "qparse"}, WolframAlpha[str, "MathematicaParse", opts]]
...
iTimeZoneToGMTString[offset_] := With[{n = N[offset]},
  If[NumberQ[n],
   Block[{tz = "Etc/GMT" <> If[n > 0, "+", ""] <> ToString[n]},
 ...
 Catch[With[{msghead = Replace[OptionValue["MessageHead"], Automatic -> EntityValue], 
   ...
   Module[{res, argscompressed, tmpcontext, timeout, args = {#,"EvalEnv"->$EvaluationEnvironment}&/@argsIn, urls, df, data, tasks}, 
    ...
    TimeConstrained[
    	...
    	timeout, 
    	Message[msghead::timeout, msghead];Throw[$Failed, $tag]],
...
Block[{result, rawparse, parseassumptions, xml, options, previouslyChosen, oldid, monitor, starttime = AbsoluteTime[], conopen=False,
		$AlphaQueryMMode = "input", hasFlag=False},
...
With[{boxversion = If[TrueQ[$LinguisticAssistantTrackQueryState], 4, If[contextinfo === Automatic, 1, 3]]},
	System`DynamicNamespace[
		...
		DeleteWithContents -> True,
		SelectWithContents -> True
	]] // If[$CloudControlEqual, AddCloudControlEqualTag @ #[[2]], #]&
...
linguisticErrorIndicator["NoParse", query_, ___] :=
	If[$CloudControlEqual, {}, Pane[suggestionsDialogButton["NoTranslations", query, "MathematicaParse"], BaselinePosition -> Scaled[0.15]]]
...
attachedMouseover2[a_, b_] := Mouseover[a, b] /; $CloudControlEqual

attachedMouseover2[a_, b_] :=
	ExpressionCell[
		DynamicModule[{attached},
			EventHandler[a, {
				"MouseEntered" :> With[{cell = EvaluationCell[]}, attached =
				MathLink`CallFrontEnd @ FrontEnd`AttachCell[
					cell,
	...
	DynamicModule[{box, attached},
		EventHandler[a, {
			"MouseEntered" :> (attached = MathLink`CallFrontEnd @ FrontEnd`AttachCell[
				box,
				Cell[BoxData[ToBoxes[EventHandler[b, "MouseExited" :> (NotebookDelete[attached]; attached =.)]]],
					Magnification -> AbsoluteCurrentValue[box, Magnification]],
				{Offset[{0,0}, 0], {Center,Center}}, {Center,Center}])
		}],
		Initialization :> (box = EvaluationBox[])
	] /; Internal`CachedSystemInformation["FrontEnd", "VersionNumber"] < 10


(* Staring with the V10 front end, there is a new ClosingActions token we can use for this *)
attachedMouseover2[a_, b_] :=
	DynamicModule[{box},
		EventHandler[a, {
			"MouseEntered" :> (MathLink`CallFrontEnd @ FrontEnd`AttachCell[
				box,
				Cell[BoxData[ToBoxes[b]],
					Magnification -> AbsoluteCurrentValue[box, Magnification]],
				{Offset[{0,0}, 0], {Center,Center}}, {Center,Center},
				"ClosingActions" -> {"MouseExit"}])
		}],
		Initialization :> (box = EvaluationBox[])
	]
...
LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "Online" -> False, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedMessageTemplate["WAStrings", "MessageTemplate:Offline"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaOffline", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "Allowed" -> False, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedMessageTemplate["WAStrings", "MessageTemplate:Disallowed"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaDisallowed", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "mparse.jsp" -> _, "$TimedOut" -> _, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedMessageTemplate["WAStrings", "MessageTemplate:QueryTimedOut"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaQueryTimedOut", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "$TimedOut" -> _, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedMessageTemplate["WAStrings", "MessageTemplate:MParseTimedOut"], "Query" -> query, querystate}}, HoldComplete @ Failure["WolframAlphaParseTimedOut", assoc]]

LinguisticAssistantMakeExpression[query_String, boxes:"None", querystate: {___, "mparse.jsp" -> _, "query.jsp" -> _, ___}, fmt_] :=
	With[{assoc = Association @ Flatten @ {"MessageTemplate" -> cachedMessageTemplate["WAStrings", "MessageTemplate:NoParse"], "Query" -> query, querystate}}, HoldComplete @ Failure["NoWolframAlphaParse", assoc]]
...
LinguisticAssistantMakeExpression[query_String, boxes:"None", fmt_] := HoldComplete[Missing["NoWolframParse", query]]

LinguisticAssistantMakeExpression[query_, boxes:"None", fmt_] := HoldComplete[Missing["NoWolframParse"]]
...
doNewFastParse[query_, contextinfo_, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_], Dynamic[querystate_]] :=
	Block[{queryurl, querylocalfile, queryobj, rawparse, parse, newassumptions = {}, $AlphaQueryMMode = "inline", QuantityUnits`Private`$WolframAlphaInputFlag = True,
			online, allowed, now, rememberState, newquerystate, $MessageList = {}},
		...
		If[online === False || allowed === False || MemberQ[{"error: mparse timedout", "error: query timedout", "error: no parse", "error: messages"}, query],
			...
			TimeConstrained[
				...
				queryobj = URLSaveAsynchronous[queryurl, querylocalfile, Null&, BinaryFormat->True, "Progress"->False];
				...
				rawparse = WolframAlpha[query, "RawMathematicaParse", InputAssumptions -> assumptions];
				...
				If[parse === None,
					...
					Quiet[WaitAsynchronousTask[queryobj], WaitAsynchronousTask::asyncobj];
					rawparse = WolframAlpha[query, "PrimaryMathematicaString", InputAssumptions -> assumptions, Method -> {"SubstituteURL" -> querylocalfile}];
		...
		If[parse === None,
			boxes = "None",
			parse = ToExpression[parse, InputForm, HoldComplete];
			Replace[parse, HoldComplete[Entity[type_String, __]] :> ($PreviousEntityType = type)];
			boxes = AlphaQueryMakeBoxes @@ parse
		];
...
doNewFastParse[query_, contextinfo_, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_], Dynamic[querystate_]] :=
	Block[{rawparse, parse, newassumptions, $AlphaQueryMMode = "inline", QuantityUnits`Private`$WolframAlphaInputFlag = True},
...
3. mparse.jsp didn't complete within 5 seconds
4. mparse.jsp completes and returns no expr; query.jsp didn't complete within 5 seconds
...
Quiet[Import["http://", "Text"]; ImportString["", "XML"]; ImportString["", "Text"]];
