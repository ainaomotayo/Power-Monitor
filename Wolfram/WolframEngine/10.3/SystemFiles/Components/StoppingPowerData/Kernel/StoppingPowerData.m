...
invalidArgumentsQ[ent_String]:=If[MemberQ[System`StoppingPowerData[],ent],False,
		Message[System`StoppingPowerData::notent,ent,System`StoppingPowerData];True]
invalidArgumentsQ[Entity[type_,ent_]]:=If[MemberQ[System`StoppingPowerData[],ent],False,
...
singleprops[ent_,prop_]:=Module[{entity=ent/.$prules,entdata,pos},
...
iStoppingPowerData[ent:(_String|_Entity),prop_]:=If[invalidArgumentsQ[ent,prop],
	$Failed,
	If[prop=!="Name",$Failed,
		Module[{entity=ent/.$prules,entdata},
			entdata=myFirst@Cases[stoppingpowerdata,{entity,_}];
			entdata[[2,2]]
		]
	]
];
iStoppingPowerData[ent_,prop_,pq_Association]:=Module[{particle,energy,speed,pqassoc=KeyMap[#/.$prules&,pq]},
...
iStoppingPowerData[ent:(_String|_Entity),prop:(_String|_EntityProperty),pq_List]:=If[
	...
	{particle=("Particle"/.(pq/.$prules))/."Particle"->Automatic,energy=("Energy"/.(pq/.$prules))/."Energy"->Automatic,
		speed=("Speed"/.(pq/.$prules)),entity=ent/.$prules, property=prop/.EntityProperty[_,x_String,___]:>x,position,
