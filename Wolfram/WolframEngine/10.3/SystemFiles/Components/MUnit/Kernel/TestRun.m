...
TestRun[testSource_, OptionsPattern[]] :=
	Module[{$loggers, ts, title, ccondition,
		...
		wasSuccessful,
		keptResObjNames = {}, sameTest, mConstraint, tConstraint},
		...
		If[title === Automatic && Switch[Part[Hold[testSource],1,0], 
                                         List, False,
                                         _, ts=testSource;Head[ts]===String],
...
noteBookFileQ[n_String]:= (StringMatchQ[n, "*.nb"]===True)
...
(*As attribute is HoldFirst, evaluating the file name*)
midTestRun[testFileName:Except[_String|_List], OptionsPattern[]]/;Quiet@StringQ[testFileName] := 
	Module[{$loggers, title, ccondition, sameTest, mConstraint, tConstraint},
		{$loggers, title, ccondition, sameTest, mConstraint, tConstraint} = OptionValue[{Loggers, TestRunTitle, CanaryCondition, SameTest, MemoryConstraint, TimeConstraint}];
		With[{evaluatedName=testFileName}, 
...
midTestRun[testFileName_String /; (!noteBookFileQ[testFileName]), OptionsPattern[]] :=
...
lowTestRun[testStream_InputStream, OptionsPattern[]] :=
	Module[{$loggers, title, ccondition,
		...
		Block[{TestLog, TestTerminate, MUnitGetFileHandler, TestSuite,
			$testIndex = 0, $dynamicTestIndex, $lexicalTestIndex, logTestResult},
					...
					Block[{$MessageNiceStringList = {}, MUnitMessageHandler = messageNiceStringHandler},
						Check[
							Quiet[
								...
								,
								(* we ignore spelling messages because they are not a report about bad syntax, they are just stupid *)
								{General::spell1, General::spell}
							]
							,
			...
			Module[{cpuTimeUsed, absTimeUsed, memoryUsed, testids},
				...
				While[True,
					...
					*)
					...
					Switch[expr,
						HoldComplete[_BeginTestSection],
						...
						_,
						testids = Cases[expr, (Rule|RuleDelayed)[TestID, id_] :> id, Infinity];
						...
						Block[{
							...
							$sectionStack = If[MatchQ[expr, HoldComplete[_DefectiveTest]], Append[$sectionStack, False], $sectionStack]},
							Outer[LogTestInfo[#1, #2, ++$lexicalTestIndex, And @@ $sectionStack]&, $loggers, testids];
							If[And @@ $sectionStack,
								(* The actual evaluation of the input and comparing of result to expected is done here. *)
								With[{currentOptions = Options[VerificationTest]},
									...
									SetOptions[VerificationTest, Sequence@@currentOptions]; 
								]
								,
								skippedTestCnt+=Length[testids]
							]
						];
