...
(*
TestID::usage =
"TestID is an option to Test and TestMatch that specifies a string used as an identifier for the test.
TestID is also a selector for TestResult."*)
...
(*
MemoryConstraint::usage =
"MemoryConstraint is an option for Test that specifes how much memory (in bytes) the test is allowed to use."
*)
...
Test[input_, expected_:True, Shortest[expectedMsgs_:{}], opts:OptionsPattern[]] :=
	Module[{testWrapper, testInputWrapper, actualOutputSetFunction, expectedOutputSetFunction, sameTest, equivalenceFunction, msgsEquivFunc,
		...
		testEquivFuncRes, msgsEquivFuncRes, evalOccurred,
		(*WRI-specific*)
			...
			Block[{
				...
				Block[{$Messages = {$bitbucket}, $MessageList = {}, MUnitMessageHandler = messageStringHandler, $MessageStringList = {}},
					...
					{testWrapper, testInputWrapper, actualOutputSetFunction, expectedOutputSetFunction, sameTest, equivalenceFunction, msgsEquivFunc,
						...
						testFailureAction, testErrorAction, timeConstraint, memoryConstraint, testTags, ntestFailureMessage, orntestFailureMessages} =
						OptionValue[Test, Automatic, {TestWrapper, TestInputWrapper, ActualOutputSetFunction, ExpectedOutputSetFunction, SameTest, EquivalenceFunction,
							...
							TimeConstraint, MemoryConstraint, TestTags, NTestFailureMessage, OrNTestFailureMessages}];					
					(*
...
$DefaultMessagesEquivalenceFunction[actualMsgs_, expectedMsgs_] :=
	Module[{expectedMsgs2},
		Block[{Message, MessageName},
			SetAttributes[Message, HoldRest];
			...
			expectedMsgs2 =
				Switch[expectedMsgs,
					...
					,
					_Alternatives,
					(* from old Testing`-style tests *)
					Map[patternizeExpectedMessage, expectedMsgs, {2}]
					,
					_,
					expectedMsgs
				];
...
*)
$DefaultExpectedMessagesWrapper[expectedMsg_MessageName] := {HoldForm[expectedMsg]}
$DefaultExpectedMessagesWrapper[expectedMsg_Message] := {HoldForm[expectedMsg]}
$DefaultExpectedMessagesWrapper[expectedMsgs_List] := HoldForm /@ Unevaluated[expectedMsgs]
$DefaultExpectedMessagesWrapper[expectedMsgs_Alternatives] := Map[HoldForm, Unevaluated[expectedMsgs], {2}]
$DefaultExpectedMessagesWrapper[expectedMsgs_] := expectedMsgs
...
179458
$NewSymbol =
	With[{old = If[OwnValues[$NewSymbol] != {}, $NewSymbol, Null&]},
		Function[
			If[StringMatchQ[#1, "TestResultObject" ~~ __] && StringMatchQ[#2, "MUnit`TestResultObjects`"] && !TrueQ[validTestResultEnv],
				(*
				warning: this does not prevent the symbol from being created
				*)
				Message[TestResultObject::valid, #2<>#1]
			];
			old[#1, #2]
		]
	]*)
