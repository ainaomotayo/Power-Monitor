...
BuildFunctionPointerCall[objData_, fun_, out_, regs_List] := 
	Module[ {name, cnt, funcData, numArgs, args, assignArgs, resArg, call, comment, convert, c = 0, r, f, afun, flowcheck = False},
		...
		, (*else *)
			If[UnsameQ[First[convertFullType[out]], Void],
				AppendTo[
					assignArgs, 
...
TypeRankReference[Register[Void, _], objData_] := {Void, 0, 0}
...
AssignTypeReference[objData_, Register[Void, _], i_, tvar_, ref_] := {CAssign[CArray[tvar, i], ToTypeEnum[Void]], CAssign[CArray[ref, i], VoidPointer[0]]}
