...
NVCCCompiler["CreateObjectFileCommands"][errMsgHd_, installation_, compilerName_, outFile_, workDir_,
								  		 ...
								  		 cleanIntermediate_, mprepOptions_, language_, opts_] :=
	...
	Module[{createCubinQ, createPTXQ, xcompilerInstallation},
		...
		];
		{
			CommandJoin[
				If[MemberQ[{"Windows", "Windows-x86-64"}, targetSystemID],
	               	CCompilerDriver`VisualStudioCompiler`Private`VCVarsCall[
	               		ParentDirectory[ParentDirectory[StringTrim[xcompilerInstallation, "\""]]],
	               		targetSystemID
	               	] <> "\n"
	               	, (* else not windows *)
	                ""
			...
			CommandJoin[
				...
				" --compiler-bindir ", xcompilerInstallation,
...
ResolveCUDALibraryPath[errMsgHd_, installation_String, targetSystemID_] :=
	QuoteFile[
		FileNameJoin[{ParentDirectory[installation], Switch[targetSystemID,
					...
					"Windows-x86-64" | "Linux-x86-64", "lib64",
...
$GPUToolsIncludes = FileNameJoin[{$InstallationDirectory, "SystemFiles", "Links", "GPUTools", "Includes"}]

GetOption[$ThisDriver, errMsgHd_Symbol, opts_List, n:"SystemLibraries",  targetSystemID_, includeMathlinkQ_] := (
	getOption[errMsgHd, opts, n] /. {
		Automatic :> 
			Join[
				Flatten[{ResolveLibraries[CCompilerDriver`CCompilerDriverBase`BaseDriver, Automatic, targetSystemID, includeMathlinkQ]}],
...
getAndCheckOption[errMsgHd_, opts_, n:"XCompilerInstallation"] :=
	Module[{targetSystemID, xcompilerInstallation},
		...
		If[MemberQ[{"Windows", "Windows-x86-64"}, targetSystemID],
			FileNameJoin[{xcompilerInstallation, "VC", "bin"}], 
...
CUDACCompilers[] := 
	With[{pths = Select[Flatten[{
			...
			CCompilerDriver`VisualStudioCompiler`VisualStudioCompiler["ResolveInstallation"][{"2010"}],
			CCompilerDriver`VisualStudioCompiler`VisualStudioCompiler["ResolveInstallation"][{"2008"}],
			CCompilerDriver`VisualStudioCompiler`VisualStudioCompiler["ResolveInstallation"][{"2005"}]
			}],
			Quiet[DirectoryQ[#]]&
...
ResolveXCompilerInstallation[Automatic, errMsgHd_, targetSystemId_String] /; $OperatingSystem === "Windows" := 
	Module[{installation},
		installation = CCompilerDriver`VisualStudioCompiler`VisualStudioCompiler["ResolveInstallation"][{"2013", "2012", "2010", "2008", "2005"}];
		If[targetSystemId =!= "Windows" &&
...
formatLibrary[lib_] /; $OperatingSystem === "Unix" || $OperatingSystem === "MacOSX" := 
	CCompilerDriver`GCCCompiler`Private`formatLibrary[lib, True]
