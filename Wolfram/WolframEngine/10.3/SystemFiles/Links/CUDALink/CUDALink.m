...
CUDAResourcesInstall[Automatic | "Web", opts:OptionsPattern[]] :=
	Module[{paclet, updateQ},
        ...
        Check[
            PacletTools`PacletGet[$CUDAResourcesPacletName, CUDALink],
            Return[$Failed]
        ];
        paclet = PacletFind[$CUDAResourcesPacletName];
        If[paclet === {},
...
CUDAResourcesUninstall[] :=
	(
		PacletUninstall /@ PacletFind[$CUDAResourcesPacletName];
...
getCUDAToolkitLibraryPath[pth_, "Linux-x86-64"] := SelectFirst[FileNameJoin[{pth, #}]& /@ {"lib", "lib64"}, DirectoryQ]
...
iCUDAFunctionLoad[errMsgHd_, prog:{_} /; !StringQ[First@prog], ___] :=
	Throw[Message[errMsgHd::invprog, First[prog]]; $Failed]
iCUDAFunctionLoad[errMsgHd_, prog_ /; (!StringQ[prog] && !ListQ[prog]), ___] :=
	Throw[Message[errMsgHd::invprog, prog]; $Failed]
iCUDAFunctionLoad[errMsgHd_, prog:{_String} /; !FileExistsQ[First@prog], ___] :=
...
iCompileCUDAKernel[errMsgHd_, prog_, OptionsPattern[]] :=
	Throw[Message[errMsgHd::invprog, prog]; $Failed]
iCompileCUDAKernel[errMsgHd_, prog:(_String | {_String}), opts:OptionsPattern[]] :=
	Module[{
			fileName, createLibraryOpts, device, useSinglePrecisionQ, arch, progText, timeStamp,
...
CUDAVolumetricDataRead[path_String?FileExistsQ, height_Integer?Positive, depth_Integer?Positive] :=
	Module[{volumeData, strm},
