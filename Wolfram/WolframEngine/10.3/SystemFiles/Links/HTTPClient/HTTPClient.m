...
getURL[exp_,head_:URLFetch]:=Block[
	{pos,url,hasProperHead},
	hasProperHead=MatchQ[Head[exp],String|System`URL|System`IPAddress];
	pos=Position[exp, _String];
	url = Extract[exp,pos];
	If[Length[url]>=1 && hasProperHead,
		url[[1]]
		,
		Message[head::invurl, exp];
		$Failed
		]
...
URLFetch[urlExp_,opts:OptionsPattern[]]/;InitializeQ[]:=
	URLFetch[urlExp,"Content",opts]	
...
URLFetch[urlExp_/;isURL[urlExp], res:(_String|_List|All), opts:OptionsPattern[]] /; InitializeQ[] :=
	Module[{url,handle, output, error, stdOpts, elements, wellFormedURL, oauth, token, args},
		url=getURL[urlExp];	
...
URLSave[urlExp_, file_String, res:(_String|_List|All):"Content", options___?OptionQ] /; deprecatedOptionQ[options] := 
	URLSave[urlExp, file, deprecatedOptionFix[URLSave, options]]  
	
URLSave[urlExp_/;isURL[urlExp,URLSave], file_String, res:(_String|_List|All):"Content", opts:OptionsPattern[]] /; InitializeQ[] :=
	Module[{handle, output, error, stdOpts, elements, wellFormedURL, oauth, token, args,url},
...
Module[
	{byteStr,bytes, contentTypeHeader, charset, mCharset,contentType}, 
	
	bytes = HTTPData[handle, "ContentData"];
		
	
	If[handle["FTP"] === True,
		byteStr = FromCharacterCode[bytes];
		Return[ImportString[byteStr,"Text"]];
		];
	
	contentTypeHeader = Select[HTTPData[handle, "Headers"], StringMatchQ[First[#], "Content-Type", IgnoreCase -> True] &];
	...
	If[MatchQ[contentTypeHeader,{{_String,_String}}],
		...
		byteStr = FromCharacterCode[bytes,mCharset];
		,
		(*If content-type header is absent, default to ISO8859-1 encoding*)
		byteStr = FromCharacterCode[bytes,"ISO8859-1"]; 
		];

	(*if content-type is of the form  <text/something> *)
	If[StringQ[contentType],
		Which[
			StringMatchQ[contentType, "*text/html*", IgnoreCase -> True],
			Return[ImportString[byteStr,{"HTML", "Source"}]];
			,
			StringMatchQ[contentType,"*text/*",IgnoreCase->True],
			Return[ImportString[byteStr,"Text",CharacterEncoding->mCharset]];
			];
		];
		
	(*Default to returning a string with encoding ISO8859-1, this encoding preseves the bytes*)
	Return[byteStr];
...
setStandardOptions[handle_CURLHandle, url_String, opts:OptionsPattern[]] := 
	Module[{multipartElements,parameters,finalURL = url, method = ToUpperCase[OptionValue["Method"]], baseURL}, 
		...
		If[MatchQ[parameters, {__}],
			If[method === "GET",
				...
				CURLOption[handle, "CURLOPT_COPYPOSTFIELDS", buildData[handle, parameters, method]]
			]
		];
		...
		Which[MatchQ[multipartElements, {Rule[{_String, _String}, _String]..}],
			...
			MatchQ[multipartElements, {Rule[{_String, _String}, {__Integer}]..}],
				CURLForm[handle, 
					#[[1]][[1]], 
					#[[1]][[2]], 
					#[[2]], 
...
Module[
	...
	Which[
		...
		,
		MatchQ[me,{Rule[{_String, _String}, {__Integer}]..}],
...
URLSaveAsynchronous[urlExp_, file_String, func:Except[_Rule|_RuleDelayed|_String], options___?OptionQ] /; deprecatedOptionQ[options] := 
	URLSaveAsynchronous[urlExp, file, func, deprecatedOptionFix[URLSaveAsynchronous, options]] 

URLSaveAsynchronous[urlExp_/;isURL[urlExp,URLSaveAsynchronous], file_String, func:Except[_Rule|_RuleDelayed|_String], opts:OptionsPattern[]] /; InitializeQ[] := 
	Module[{handle, stdOpts, error, oauth, token, args, output,url},
		url=getURL[urlExp];
		If[OptionValue["OAuthAuthentication"] =!= None,
			oauth = OptionValue["OAuthAuthentication"];
