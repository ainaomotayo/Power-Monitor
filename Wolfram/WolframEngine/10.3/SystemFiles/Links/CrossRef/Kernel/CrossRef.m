...
(*crossrefcookeddata[req_, id_]:=crossrefcookeddata[req, id, {}]*)
crossrefcookeddata[prop_,id_,rules___Rule]:=crossrefcookeddata[prop,id,{rules}]
...
crossrefcookeddata[prop:("WorksList"|"WorksDataset"), id_, args_] := Module[{license,hft,dateRange,startDate,endDate,filterParam={},reqName,invalidParameters,params={},sort,query,limit=20,maxPerPage=1000,startIndex,
																	...
																	funderID,typeID,ownerPrefix,memberID,issn},
	invalidParameters = Select[Keys[args],!MemberQ[{"MaxItems","StartIndex","SortBy","Query",
													"FunderID","TypeID","OwnerPrefix","MemberID","ISSN","PublishedDate","License","HasFullText"},#]&]; 
	...
	If[KeyExistsQ[args,"PublishedDate"],
	(
		dateRange = "PublishedDate" /. args;
		...
		Switch[dateRange,
			...
			_,
			(
				Message[ServiceExecute::nval,"PublishedDate","CrossRef"];	
				Throw[$Failed]
			)
		];
	...
	result = ReplaceAll[result,Rule["Author",a_]:>Rule["Author",ReplaceAll[#,Rule[x_,y_]:>Rule[camelCase[x],y]]&/@a]];
	result = ReplaceAll[result,Rule["Link",a_]:>Rule["Link",ReplaceAll[#,Rule[x_,y_]:>Rule[camelCase[x],y]]&/@a]];
	result = ReplaceAll[result,Rule["License",a_]:>Rule["License",ReplaceAll[#,Rule[x_,y_]:>Rule[camelCase[x],y]]&/@a]];
	result = ReplaceAll[result,Rule["Funder",a_]:>Rule["Funder",ReplaceAll[#,Rule[x_,y_]:>Rule[camelCase[x],y]]&/@a]];
...
crossrefcookeddata["MemberInformation", id_, args_] := Module[{rawdata, invalidParameters,member,result},
		...
		result = ReplaceAll[result,Rule["Counts",l_]:>Rule["Counts",ReplaceAll[l,Rule[x_,y_]:>Rule[camelCase[x],y]]]];
		result = ReplaceAll[result,Rule["Breakdowns",l_]:>Rule["Breakdowns",ReplaceAll[l,Rule[x_,y_]:>Rule[camelCase[x],y]]]];
		result = ReplaceAll[result,Rule["Coverage",l_]:>Rule["Coverage",ReplaceAll[l,Rule[x_,y_]:>Rule[camelCase[x],y]]]];
		result = ReplaceAll[result,Rule["Prefix",l_]:>Rule["Prefix",ReplaceAll[l,Rule[x_,y_]:>Rule[camelCase[x],y]]]];
		result = ReplaceAll[result,Rule["Flags",l_]:>Rule["Flags",ReplaceAll[l,Rule[x_,y_]:>Rule[camelCase[x],y]]]];
		result = ReplaceAll[result,Rule["Id",y_]:>Rule["ID",y]];
...
crossrefcookeddata[prop:("FunderList"|"FunderDataset"), id_, args_] := Module[{invalidParameters,params={},query,limit=20,maxPerPage=1000,startIndex,
																	calls,residual,progress=0,data,rawdata,totalResults,items={},result,sample},
	invalidParameters = Select[Keys[args],!MemberQ[{"MaxItems","StartIndex","Query"},#]&]; 
...
crossrefcookeddata[prop:("MemberList"|"MemberDataset"), id_, args_] := Module[{filterParam={},invalidParameters,params={},query,limit=20,maxPerPage=1000,startIndex,
																	calls,residual,progress=0,data,rawdata,totalResults,items={},result,ownerPrefix},
	invalidParameters = Select[Keys[args],!MemberQ[{"MaxItems","StartIndex","Query","OwnerPrefix"},#]&]; 
...
crossrefcookeddata[prop:("LicenseList"|"LicenseDataset"), id_, args_] := Module[{invalidParameters,params={},query,limit=20,maxPerPage=1000,
																	data,rawdata,totalResults,items={},result},
	invalidParameters = Select[Keys[args],!MemberQ[{"MaxItems","Query"},#]&]; 
...
crossrefcookeddata[prop:("JournalList"|"JournalDataset"), id_, args_] := Module[{invalidParameters,params={},query,limit=20,maxPerPage=1000,startIndex,
																	calls,residual,progress=0,data,rawdata,totalResults,items={},result},
	invalidParameters = Select[Keys[args],!MemberQ[{"MaxItems","StartIndex","Query"},#]&]; 
