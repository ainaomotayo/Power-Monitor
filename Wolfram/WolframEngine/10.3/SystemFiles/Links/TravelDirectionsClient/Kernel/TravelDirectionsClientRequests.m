...
TravelDirections::servermessage = TravelTime::servermessage = TravelDistance::servermessage = "Server message: `1`";
...
callGetDirections[fun_, locations_List, opts_:{}] :=
	...
	If[ListQ@res, messages = EntityFramework`Private`replaceWithDefault["Messages", res, $Failed] /. WolframAlphaClient`Private`$FromMWARules];
	If[ListQ@res, res = EntityFramework`Private`replaceWithDefault["Result", res, $Failed]];
	If[ListQ@messages,
		Scan[(# /. {
...
TravelDistance[locations:{(_String|_Entity|_GeoPosition)..}, opts:OptionsPattern[]] :=
    travelDirectionsInternal[TravelDistance, locations, opts]["TravelDistance"]
...
TravelTime[locations:{(_String|_Entity|_GeoPosition)..}, opts:OptionsPattern[]] :=
    travelDirectionsInternal[TravelTime, locations, opts]["TravelTime"]
