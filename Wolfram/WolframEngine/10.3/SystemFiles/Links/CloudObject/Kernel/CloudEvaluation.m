...
CloudEvaluate[expr_, head_:Function[##]] /; TrueQ[$CloudEvaluation] := head[expr]
CloudEvaluate[expr_, head_:Function[##]] := 
	Replace[
		execute[
			$CloudBase, 
			"POST", 
			{"evaluations"}, Body -> exprToStringBytesWithSaveDefinitions[Unevaluated[expr]]
		], {
			...
			result_ :> (checkError[result, CloudEvaluate]; $Failed) 
		}
	]

handleSuccessfulEvaluation[{_, bytes_List}, rest___] := 
	handleSuccessfulEvaluation[FromCharacterCode[bytes], rest]
	...
	LegacyCloudFunction[expr_, headwrap_][args___] :=
	    Module[{co},
	        Block[{formalargs}, 
	            ...
	            ], CloudObject[], expressionMimeType["CloudEvaluation"], SaveDefinitions -> True, IconRules -> {}];
