...
simpleLog[args___] := Print[
    ...
    "\t", 
    Apply[Sequence, {args} /. s_StringForm :> ToString[s]]
];
...
resolveDocGenResource[res_] := Check[
    Replace[res, {
        ...
        t:Except[_Delayed|_CloudObject|None] :> CloudObject`Private`deleteable@CloudPut[t, SaveDefinitions -> True]
    }],
    Throw[$Failed, $tag]
];
...
(* Not officially supported yet, but named time zones are reported in ScheduledTaskInformation
 * and can feed back in during updates
 *)
...
iCloudDeployDocumentGenerator[
    ...
    {cloud, uuid, name} = getCloudAndUUIDOrPath[co] /. None -> Null;
	name = Replace[name, {
    ...
    If[FileExistsQ[co] && MemberQ[$docGenMimeTypes, CloudObjectInformation[co, "MimeType"]],
        ...
        Check[With[{dgInfo = CloudObject`Private`iCloudDocumentGeneratorInformation[co]}, 
				...
				existingOpts = ReplaceAll[FilterRules[
					Join[Normal@dgInfo, Normal@dgInfo["Task"]],
				    Options[DocumentGenerator]
				], {Rule[EpilogFunction, e_] :> Rule[EpilogFunction, preexisting[e]]}];
    ...
    params = {"properties" -> ExportString[unpresentifyDocGenMeta[FilterRules[opts, Except[Options[ScheduledTask]]]], "JSON", "Compact" -> True],
    	"path" -> Replace[name, Null -> ""],
    	"task" -> (taskJson = With[{expr = uuid, timespec = sched},
    		generateTaskJson[ScheduledTask[expr, timespec], {name, Null}, cronned,
...
(* RemoveScheduledTask *) 
iCloudRemoveDocumentGenerator[obj_CloudObject, mh_:RemoveScheduledTask] := (iCloudRemoveDocumentGenerator[safeCloudAndUUIDFetch[obj, mh], mh]; obj)
iCloudRemoveDocumentGenerator[{cloud_String, uuid_String}, mh_:RemoveScheduledTask] := Module[
    {json},
    json = Replace[execute[cloud, "DELETE", {"reports", uuid}], {
        HTTPError[404, ___] :> (Message[ScheduledTask::tasknf, cloudObjectFromUUID[uuid]]; Throw[$Failed, $tag])
        , {_String, content_List} :> ($lastInfoJSON = FromCharacterCode[content])
        , other_ :> (Message[mh::srverr]; Message[ScheduledTask::norm]; Throw[$Failed, $tag])
    }];
    uuid
];
...
gatherDeliveryFunction[n:{{__}...}] := SortBy[
    Sort["email" /. #] -> ToExpression["format" /. #[[1, -1]]] & /@ GatherBy[n, "format" /. # &],
    Last
];
...
$docGenDenormalizationRules = {
    ...
    Rule[tag:"creationDate"|"lastModificationDate"|"lastRunDate", t:(_DateObject|_?NumericQ)] :> 
        Rule[tag, Round[1000*ToUnixTime[t]]],
...
iCloudDocumentGeneratorInformation[{cloud_String, uuid_String}, mh_:DocumentGeneratorInformation] := Module[
    ...
    With[{msghd = mh},
        ...
        Check[raw = Lookup[ImportString[json, "JSON"], "report"],
            Message[mh::srverr];
            Throw[$Failed, $tag]
        ]
    ];
...
DocumentGenerators[] := Module[
    {res = Catch[iCloudDocumentGenerators[], $tag]},
    res
]

iCloudDocumentGenerators[] := Module[
    {raw, med, json, msghd = DocumentGenerators},
    ...
    With[{mh = msghd},
        ...
        Check[raw = Lookup[ImportString[json, "JSON"], "reports", {Missing[]}],
            Message[mh::srverr];
            Throw[$Failed, $tag]
        ];
    ];
    ...
    med[1] = Lookup[raw, {"uuid", "schedule"}, Missing[]];
    (* This sorts by next run date, soonest first. *)
    med[2] = With[{culled = Reverse @ Quiet @ SortBy[DeleteCases[med[1], {___, Missing[], ___}], Lookup[Last[#], "nextTimestamp"] &]},
        culled
    ];
