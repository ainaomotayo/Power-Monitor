...
$EvaluationEnvironmentParameters = <| |> (* default in desktop environment *)
...
HTTPRequestData[arg_] := 
    Module[{success = True, res},
        Replace[$HTTPRequest, {
            assoc_?AssociationQ :>
                (res = Lookup[assoc, translateHTTPRequestData[arg], (success=False; HTTPRequestDataMissing)];
                 If[ !success,
...
$defaultEncoding = "Unicode"; (* per RFC-2045, etc., want it untranslated,
                                 i.e. ASCII, but perhaps more lenient.
                                 This could also be "ISO8859-1". *)
ToCharacterEncoding[enc_String | {enc_String}, default_String:$defaultEncoding] :=
    oToCharacterEncoding[enc, default];
ToCharacterEncoding[{}, default_String:$defaultEncoding] := $defaultEncoding;
ToCharacterEncoding[encs: {enc1_String, __String}, default_String:$defaultEncoding] :=
    oToCharacterEncoding[enc1, ToCharacterEncoding[Rest[encs], default]];
call:ToCharacterEncoding[args___] := With[{},
    System`Private`Arguments[call, {1,2}]; (* to issue a message for wrong number of args*)
    $Failed /; False] (* always returns unevaluated *)

normalizeEncoding[enc_String] := StringReplace[enc, "-" -> ""];

SetAttributes[oToCharacterEncoding, HoldRest]; (* don't prematurely evaluate the default *)
oToCharacterEncoding["Unicode", _] = "Unicode"; (* short-circuit common case *)
If[ MemberQ[$CharacterEncodings, "UTF-8"], (* defend against "UTF8" or other conventions *)
    oToCharacterEncoding["UTF-8", _] = "UTF-8"; (* short-circuit presumed common case *)
];
oToCharacterEncoding[enc_String, default_ (* _String | _ToCharacterEncoding *)] :=
    With[{normalizedEnc = normalizeEncoding[enc]},
        SelectFirst[
            $CharacterEncodings
            ,
            StringMatchQ[normalizeEncoding[#], normalizedEnc, IgnoreCase->True]&
            ,
            (* the following is not evaluated unless needed because SelectFirst is HoldRest *)
            If[ StringMatchQ[normalizedEnc, "usascii", IgnoreCase->True]
                && MemberQ[$CharacterEncodings, "ASCII"],
                "ASCII"
                ,
                oToCharacterEncoding[default, $defaultEncoding]
            ]
        ]
    ];
