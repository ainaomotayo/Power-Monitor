...
ToEntityColorData[Red] = Entity["Color",{"Mathematica","Red"}]
ToEntityColorData[Green] = Entity["Color",{"Mathematica","Green"}]
ToEntityColorData[Blue] = Entity["Color",{"Mathematica","Blue"}]
ToEntityColorData[Black] = Entity["Color",{"Mathematica","Black"}]
ToEntityColorData[White] = Entity["Color",{"Mathematica","White"}]
ToEntityColorData[Gray] = Entity["Color",{"Mathematica","Gray"}]
ToEntityColorData[Cyan] = Entity["Color",{"Mathematica","Cyan"}]
ToEntityColorData[Magenta] = Entity["Color",{"Mathematica","Magenta"}]
ToEntityColorData[Yellow] = Entity["Color",{"Mathematica","Yellow"}]
ToEntityColorData[Brown] = Entity["Color",{"Mathematica","Brown"}]
ToEntityColorData[Orange] = Entity["Color",{"Mathematica","Orange"}]
ToEntityColorData[Pink] = Entity["Color",{"Mathematica","Pink"}]
ToEntityColorData[Purple] = Entity["Color",{"Mathematica","Purple"}]
...
ToEntityColorData[other_] := CompoundExpression[
	Message[ToEntity::noentp, other, "Color"],
	$Failed
]
...
makeMFDPatternEntityPairs[] := Module[
	...
	p1 = Cases[
		p0,
		{Verbatim[HoldPattern][s_Symbol?(Context[#] =!= "System`" &)[args___]], ent_} :> {HoldPattern[_Symbol?(And[AtomQ[#], SymbolName[#] === SymbolName[s]] &)[args]], ent},
...
iToEntity[s_Symbol] /; MemberQ[WLSymbolNames, ToString[s]] := iToEntityWolframLanguageSymbolHandler[s]
...
iToEntity[e_, type_] := CompoundExpression[
	Message[ToEntity::noentp, e, type],
	$Failed
]

iToEntity[args___] := CompoundExpression[
	System`Private`Arguments[FromEntity[args], {1, 2}],
	$Failed
]
...
getGraphHash[g_] := With[{cf=CanonicalForm[g]},
	If[needsLegacyHashQ[],
...
iToEntityGraphHandler[other_] := CompoundExpression[
	Message[ToEntity::noentp, other, "Graph"],
	$Failed
]
...
iToEntityCharacterHandler[other_] := CompoundExpression[
	Message[ToEntity::noentp, other, "Character"],
	$Failed
]
...
iToEntityWolframLanguageSymbolHandler[s_Symbol] /; MemberQ[WLSymbolNames, ToString[s]] := With[{sas = wlSymbol2String[s]},
...
iToEntityWolframLanguageSymbolHandler[other_] := CompoundExpression[
	Message[ToEntity::noentp, other, "WolframLanguageSymbol"],
	$Failed
]
