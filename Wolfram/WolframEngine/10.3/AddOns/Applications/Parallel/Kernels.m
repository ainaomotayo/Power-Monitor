...
checkVersion[sub_, {subKernel_, subLanguageVersion_, subMathVersion_}] :=
	Which[
		...
		!TrueQ[subKernel], (* False or junk *)
			...
			$Failed,
		TrueQ[subLanguageVersion === Null || subLanguageVersion < Parallel`Private`$ParallelLanguageVersion],
...
ClearKernels[] := 
  Module[{res},
	...
	(kernelSwallow[$kernels]; res = kernelInitialize[#;, $kernels])& /@ $clientCode;

	(* user init code, see constructor *)
	If[ValueQ[$InitCode],  With[{clientCode=$InitCode}, res = kernelInitialize[ ReleaseHold[clientCode];, $kernels ]]; ];
...
If[ TrueQ[Parallel`Debug`$Debug], (* debug version *)
    kernelInitialize[cmd_, links_List] :=
        Block[{Parallel`Debug`Private`$hideVals=True}, (linkWriteRaw[#, EvaluatePacket[cmd]]&) /@ links ];
, (* else non-debug version *)
    kernelInitialize[cmd_, links_List] := (linkWriteRaw[#, EvaluatePacket[cmd]]&) /@ links;
]
...
If[ TrueQ[Parallel`Debug`$Debug], (* debug version *)
    kernelEvaluate[cmd_, links_List] :=
        Block[{Parallel`Debug`Private`$hideVals=True}, receiveCatch /@ (sendCatch[#, cmd]&) /@ links ];
, (* else non-debug version *)
    kernelEvaluate[cmd_, links_List] := receiveCatch /@ (sendCatch[#, cmd]&) /@ links;
]
...
With[{opts = Select[Options[$Messages], 
                    MemberQ[{BinaryFormat, FormatType, PageWidth, PageHeight, TotalWidth, TotalHeight, CharacterEncoding, NumberMarks}, First[#]] &]},
	AppendInitCode[holdCompound[ Unprotect[MessagePacket], Clear[MessagePacket],
		SetOptions[$Messages, Sequence@@opts] ]];
]
...
feedbackHandler["tick"] := (currentCount++; grandCount++; If[grandCount>grandTotalCount, grandTotalCount=grandCount]; Pause[feedbackPause])
...
clearAutolaunch[] := (
	List @@ ((DownValues[#] = $autoDownValues[#]; Protect[#])& /@ $autoSymbols);
