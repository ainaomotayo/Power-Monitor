...
For a very subtle reason we cannot use Replace[tasks, ..., {1}]
*)

Receive[kernels:{___kernel}, h_:identity] :=
    Module[{tasks = kernels, ntasks, mtasks, res},
        mtasks = ntasks = Length[tasks];
        While[ ntasks>0,
            tasks = Replace[#, k_kernel /; (res=ReceiveIfReady[k, HoldComplete]) =!= $NotReady :> (ntasks--; res)]& /@ tasks;
            If[ntasks==mtasks, Pause[Parallel`Settings`$BusyWait], mtasks = ntasks]; (* no spinning *)
        ];
        h @@@ tasks (* replace the HoldComplete wrappers. Is immune to $Failed *)
    ]

(* junk in the list, possible failed kernels, tread carefully *)

Receive[junk_List, h_:identity] := Receive[#, h]& /@ junk
...
ParallelEvaluate[cmd_, ids:(_Integer|{__Integer})] := ParallelEvaluate[cmd, KernelFromID[ids]]

(* default: use all kernels *)

ParallelEvaluate[cmd_] := ParallelEvaluate[cmd, Kernels[]]

(* second argument is a kernel or a list of kernels *)

ParallelEvaluate[cmd_, k:(_kernel|{___kernel})] :=
	CheckAbort[
		Receive[Send[k, cmd]]
		, AbortKernels[]; Abort[]
	]
...
(* note we have to abort ALL kernels... *)
...
updatedDefs[Language`DefinitionList[args___], incr_] := Language`DefinitionList@@Evaluate[updatedDefs[#,incr]&/@{args}]
...
(* augment list of excluded contexts with parallel-related ones *)

SetOptions[Language`ExtendedFullDefinition, "ExcludedContexts" ->
	Union["ExcludedContexts" /. Options[Language`ExtendedFullDefinition],
		{"ClusterIntegration", "GUIKit" "KernelLaunch", "MEET", "MEETDebug",
		 "SubKernels", "Workbench", "WorkbenchCallHandler"}]
]

`$ourExcludedContexts = "ExcludedContexts" /. Options[Language`ExtendedFullDefinition]; (* initial value *)

addBadContext[s_] := ($ourExcludedContexts = Union[$ourExcludedContexts, {s}])
...
DistDefs[expr_, exc_List, ctxts_] := Module[{updates},
	...
	updates = Language`ExtendedFullDefinition[expr, "ExcludedContexts"->$ourExcludedContexts];
...
Protect[ DistributeDefinitions,ParallelNeeds,$DistributedDefinitions,DistributedContexts ]
