...
addCommands[{cmds___}] := (
	$protected    = Join[ $protected, Unprotect[cmds] ];
...
Scan[Function[cmd,
    cmd/: tryCombine[cmd[str_, params__], opts___] := ParallelCombine[cmd[#, params]&, str, opts]
    ], commands]
...
Apply[Function[{cmd,comb},
    cmd/: tryCombine[cmd[str_, params__], opts___] := ParallelCombine[cmd[#, params]&, str, comb, opts]
...
Map/: tryCombine[Map[f_, str_], opts___] := ParallelCombine[Map[f, #]&, str, opts]
Map/: tryCombine[Map[f_, str_, level_?goodLevel], opts___] := ParallelCombine[Map[f,#,level]&, str, opts]

(* scan: throw away the result (Null) *)
Scan/: tryCombine[Scan[f_, str_], opts___] := ParallelCombine[Scan[f, #]&, str, Null&, opts]
Scan/: tryCombine[Scan[f_, str_, level_?goodLevel], opts___] := ParallelCombine[Scan[f,#,level]&, str, Null&, opts]

(* MapIndexed, only default level 1, eval str only once *)

MapIndexed/: tryCombine[MapIndexed[f_, str_], opts___] :=
	With[{estr=str}, ParallelCombine[Function[trsp,Apply[f,trsp,{1}]], Transpose[{estr, List /@ Range[Length[estr]]}], opts]]
...
Apply/: tryCombine[Apply[f_, str_, level_?goodLevel], opts___] := ParallelCombine[Function[s,Apply[f,s,level]], str, opts]
...
Inner/: tryCombine[Inner[f_, str1_, str2_, g_Symbol:Plus], opts___]/;ArrayDepth[str1]==1 && MemberQ[Attributes[g],Flat] :=
	ParallelCombine[g@@f@@@#&,Transpose[{str1,str2}], g, opts]
...
Inner/: tryCombine[Inner[f_, str1_, str2_, g_], opts___]/;ArrayDepth[str1]==1 :=
	ParallelCombine[g@@f@@@#&,Transpose[{str1,str2}], opts]
...
Pick/: tryCombine[Pick[list_, sel_, patt_:True], opts___] :=
	Module[{elist=list, esel=sel}, With[{h=Head[elist], epatt=patt},
		ParallelCombine[Pick[h @@ #1, #2, epatt] & @@ Transpose[#] &, Transpose[{List@@elist, esel}]]/;
		Length[elist]===Length[esel]
	]]
