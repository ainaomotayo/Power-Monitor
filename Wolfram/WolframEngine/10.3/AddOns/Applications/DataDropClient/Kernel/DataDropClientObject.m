...
setdatabinsettings[id_, as_Association]:=Block[{settings, res},
	settings=KeyMap[#/.Permissions->"Permissions"&,as];
...
Databin/:TimeSeries[db_Databin]:=db["TimeSeries"]
Databin/:Values[db_Databin]:=db["Values"]
Databin/:Normal[db_Databin]:=db["Entries"]
Databin/:Keys[db_Databin]:=db["Keys"]
Databin/:Get[db_Databin]:=db["FullEntries"]
...
databinAboveFoldTypesetting[fullid_,None|"Unnamed", databin_]:=If[limitedbinQ[databin],
                        {BoxForm`SummaryItem[{"Short ID: ", getShortBinID[fullid]}],
                            BoxForm`SummaryItem[{"Total entry count: ", Dynamic[Lookup[Replace[
                            datadropclientcache[{"DatabinStats", fullid}],Except[_Association]->{},{0}],"EntryCount",0]]}],
                            typesetlimits[getBinLimits[databin]]},
                        {BoxForm`SummaryItem[{"Short ID: ", getShortBinID[fullid]}],
                            BoxForm`SummaryItem[{"Entry count: ", Dynamic[Lookup[Replace[
                            datadropclientcache[{"DatabinStats", fullid}],Except[_Association]->{},{0}],"EntryCount",0]]}]}
                    ]
...
databinBelowFoldTypesetting[fullid_,id_,name:(None|"Unnamed"), databin_]:=
        {
            ...
            BoxForm`SummaryItem[{If[limitedbinQ[databin],"Total size: ","Size: "], Dynamic[
                Round[Lookup[Replace[datadropclientcache[{"DatabinStats", fullid}],Except[_Association]->{},{0}],"Size",0]],1/10]}],
            ...
            (*,
            BoxForm`SummaryItem[{"Sample: ", Dynamic[Replace[datadropclientcache[{"DatabinRecentEventSeries", fullid}],Except[_TemporalData]->Missing[],{0}]]}, form] *)
        }

databinBelowFoldTypesetting[fullid_,id_,_, databin_]:=
        {
            BoxForm`SummaryItem[{"Short ID: ", getShortBinID[fullid]}],
            BoxForm`SummaryItem[{"UUID: ", fullid}],
            BoxForm`SummaryItem[{"ShortURL: ", Hyperlink[getBinURL[fullid]]}],
