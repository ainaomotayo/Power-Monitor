...
$props={"MaxFrequency","MaxFrequencyColor","MaxWavelength","MaxWavelengthColor","MeanFrequency","MeanFrequencyColor",
...
iPlanckRadiationLaw[t_,"SpectralPlot"]:=Module[{temperature=UnitConvert[t,"Kelvins"],
   ...
   Plot[factor2/((-1 + E^(factor1/w))*w^5),{w,0,10*lpeak},Frame->True,Axes->False,PlotRange -> All,
...
iPlanckRadiationLaw[t_,"MaxWavelengthColor"]:=Module[{wavelength=iPlanckRadiationLaw[t,"MaxWavelength"]},
...
iPlanckRadiationLaw[t_,{f1_,f2_},"Frequency"]:=Module[{temperature=QuantityMagnitude[UnitConvert[t,"Kelvins"]],
 	frequency1=QuantityMagnitude[UnitConvert[f1,"Hertz"]],
   	...
   	UnitConvert[Quantity[2, "PlanckConstant"*"Hertz"^4/"SpeedOfLight"^2/"Steradians"]*
   		Quiet[NIntegrate[f^3/((-1 + E^(factor*f/temperature))),{f,frequency2,frequency1}],{NIntegrate::ncvb}],
   		"Watts"/"Meters"^2/"Steradians"]
]
iPlanckRadiationLaw[t_,w_,"Wavelength"]:=Module[{temperature=UnitConvert[t,"Kelvins"],wavelength=w},
   If[Not[NumericQ[QuantityMagnitude[t]]&&(NumericQ[QuantityMagnitude[w]]||Abs[QuantityMagnitude[w]]===Infinity)],Return[$Failed]];
...
iPlanckRadiationLaw[t_,f_,"Frequency"]:=Module[{temperature=UnitConvert[t,"Kelvins"]},
   If[Not[NumericQ[QuantityMagnitude[t]]&&(NumericQ[QuantityMagnitude[f]]||Abs[QuantityMagnitude[f]]===Infinity)],Return[$Failed]];
