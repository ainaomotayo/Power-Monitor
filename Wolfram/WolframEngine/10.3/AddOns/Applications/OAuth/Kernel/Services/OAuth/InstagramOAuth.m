		...
		"UserData",
			(Association[
				Replace[
					...
					HoldPattern[Rule[a_,b_]]:>Rule[camelcase[a],b],Infinity]
				]&),
...
instagramcookeddata[prop:("TaggedMedia"|"TaggedMediaURLs"|"TaggedMediaIDs"),id_,args_]:=Module[
	{rawdata, resolution,data, videoform,urls, params},
	params=filterparameters[args,getallparameters["RawRecentTaggedMedia"]];
	params=params/. HoldPattern["Tag" -> x_String /; StringMatchQ[x, "#*"]] :> "Tag" -> StringDrop[x, 1];
	rawdata=OAuthClient`rawoauthdata[id,"RawRecentTaggedMedia",params];
	resolution=getquality[{args},"Low"];
...
instagramcookeddata[prop:("FollowerIDs"|"Followers"|"FollowerCount"),id_,args_]:=Module[
	...
	*)
	...
	If[Length[data]=!=1,Return[prop/.{"FollowerCount"->0,"FollowerIDs"|"Followers"->{}}]];
	Switch[prop,
		"FollowerCount",StringCount[data[[1]], "\"username\""],
		"FollowerIDs",StringCases[data[[1]], "\"id\":\"" ~~ x : Shortest[__] ~~ "\"" :> x],
		"Followers",OAuthClient`Private`fromunicode[StringCases[data[[1]], "\"full_name\":\"" ~~ x : Shortest[__] ~~ "\"" :> x]]
	]
	 (* end workaround *)
...
instagramcookeddata[prop:("FollowingIDs"|"Followings"|"FollowingCount"),id_,args_]:=Module[
	...
	*)
	...
	Switch[prop,
		...
		"Followings",OAuthClient`Private`fromunicode[StringCases[data[[1]], "\"full_name\":\"" ~~ x : Shortest[__] ~~ "\"" :> x]]
	]
	 (* end workaround *)
...
instagramcookeddata[prop:("LikeCount"|"LikeIDs"|"Likes"),id_,args_]:=Module[
	...
	*)
	...
	Switch[prop,
		...
		"Likes",OAuthClient`Private`fromunicode[StringCases[data[[1]], "\"full_name\":\"" ~~ x : Shortest[__] ~~ "\"" :> x]]
	]
	 (* end workaround *)
